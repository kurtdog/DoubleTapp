Shader "PlanetaryTerrain/BumpedPlanetPoleTexture"
{
	Properties 
	{
...
_PoleTexture("_PoleTexture", 2D) = "white" {}
_PoleNormal("_PoleNormal", 2D) = "white" {}
...
Fog{
		...
		CGPROGRAM
#pragma surface surf BlinnPhongEditor  fullforwardshadows
#pragma target 3.0
#pragma glsl
...
float4 _PolarColor;
sampler2D _PoleTexture;
sampler2D _PoleNormal;
...
// pole texture
float4 PoleTex = tex2D(_PoleTexture, IN.uv_PoleTexture.xy) * _PolarColor;
float polarity = saturate((IN.color.y - _Polarity) * _PolarStrenght);
Add0 = lerp(Add0, PoleTex, polarity);

// pole normal
float4 PoleNormal = tex2D(_PoleNormal, IN.uv_PoleTexture.xy);
Lerp5 = lerp(Lerp5, PoleNormal, polarity);
